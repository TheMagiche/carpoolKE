import 'package:carpoolke/models/messages.dart';
import 'package:carpoolke/views/home/chat/view_image.dart';
    ...
    getUID().then((user) {
      setState(() {
        ...
        getSenderPhotoUrl(_senderuid).then((snapshot) {
          setState(() {
            senderPhotoUrl = snapshot['userImage'];
            senderName = snapshot['fname'] + snapshot['lname'];
          });
        });
        getReceiverPhotoUrl(widget.receiverUid).then((snapshot) {
          setState(() {
            receiverPhotoUrl = snapshot['userImage'];
            receiverName = snapshot['fname'] + snapshot['lname'];
          });
        });
      });
    });
